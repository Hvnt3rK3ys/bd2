...
FROM Libros 
WHERE strftime('%Y', fecha_publicacion) > '2000';
...
FROM Autores A
JOIN Libros L ON A.autor_id = L.autor_id
WHERE strftime('%Y', L.fecha_publicacion) < '1990'
AND L.cantidad_disponible >= 1;

-- 10. Autores con libros prestados >3 veces y disponibles actualmente
SELECT A.nombre,
       COUNT(DISTINCT CASE WHEN P.prestamo_id IS NOT NULL THEN L.libro_id END) AS libros_prestados,
